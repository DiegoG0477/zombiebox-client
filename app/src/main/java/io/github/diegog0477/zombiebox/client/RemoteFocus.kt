...
class RemoteFocus {
    ...
    fun rebuild(rows: List<Pair<String, ViewGroup>>, restore: Boolean) {
        views.clear()
        model.rebuild(rows.map { (id, group) ->
            val keys = ArrayList<String>()
            for (i in 0 until group.childCount) {
                val child = group.getChildAt(i)
                if (!child.isFocusable || child.visibility != View.VISIBLE || !child.isEnabled) continue
                val key = child.tag as? String ?: "$id:$i"
                child.tag = key
                keys.add(key); views[key] = child
    ...
    fun restore() { model.selected?.let { focus(it) } }
