...
class YouTubeReceiverViewModel(
    private val repository: YouTubeReceiverRepository,
    private val execute: (() -> Unit) -> Unit,
    private val deliver: (() -> Unit) -> Unit,
) {
    var receiver: YouTubeReceiver? = null
        private set
    ...
    fun tick() {
        ...
        val run = generation
        val sent = ready
        val feedback = sent ?: latest.copy(commandId = "")
        execute {
            try {
                ...
                val accepted =
                    try {
                        repository.feedback(active.id, feedback)
                        true
                    } catch (_: Exception) {
                        false
                    }
                ...
                deliver {
                    if (!closed && run == generation) {
                        busy = false
                        failed = false
                        if (ready === sent && (accepted || result.command?.id != sent?.commandId))
                            ready = null
                        receiver = result
                        observer?.invoke()
                        val incoming = result.command
                        if (incoming != null && incoming.id != dispatched) {
                            dispatched = incoming.id
                            pending = incoming
                            buffered = false
                            command?.invoke(incoming)
                        }
                    }
                }
            } catch (_: Exception) {
    ...
    fun playerState(state: String, position: Int, duration: Int) {
        ...
        val success =
            when (current.action) {
                ...
                "stop" -> state == "STOPPED"
                "seek" ->
                    state in listOf("PLAYING", "PAUSED") &&
                        kotlin.math.abs(position - current.positionMs) < 1500
                else -> false
            }
        if (success || state == "FAILED") {
            ready = latest.copy(commandId = current.id, success = success)
            pending = null
        }
    }

    fun volumeApplied(id: String, volume: Int, muted: Boolean, success: Boolean) {
        if (pending?.id != id) return
        if (success) latest = latest.copy(volume = volume, muted = muted)
        complete(success, id)
    }

    fun complete(success: Boolean, id: String? = null) {
        val current = pending ?: return
