...
class GatewayProbeRepository(private val api: GatewayApi): ProbeRepository {
    ...
    override fun save(results: List<ProbeResult>) {
        val previous = api.request("GET", "/v1/device").getJSONObject("capabilities").optJSONArray("probes") ?: JSONArray()
        val replaced = results.map { it.id }.toSet()
        val values = JSONArray()
        for (i in 0 until previous.length()) if (previous.getJSONObject(i).optString("id") !in replaced) values.put(previous.getJSONObject(i))
        results.forEach {
            values.put(JSONObject().put("id", it.id).put("status", it.status).put("prepareMs", it.prepareMs)
                .put("firstFrameMs", it.firstFrameMs).put("positionMs", it.positionMs).put("completed", it.completed).put("droppedOrStalled", it.stalled))
        }
        api.request("PUT", "/v1/device/capabilities", JSONObject().put("capabilitiesVersion", 1).put("deviceId", api.device).put("probes", values))
    }
}
