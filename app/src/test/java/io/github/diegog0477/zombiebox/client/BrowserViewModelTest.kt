...
class BrowserViewModelTest {
    ...
    @Test fun closeBeforeCreationReturnsReleasesLateSession(){
        val repository=Repository();val work=ArrayList<()->Unit>()
        val model=BrowserViewModel(repository,{work.add(it)},{it()})
        model.open("https://example.org");model.close();work.removeAt(0)()
        assertEquals(listOf("browser"),repository.stopped);assertEquals("",model.state.session)
    }
    @Test fun closeBetweenWorkAndDeliveryStillReleasesSession(){
        val repository=Repository();val work=ArrayList<()->Unit>();val delivery=ArrayList<()->Unit>()
        val model=BrowserViewModel(repository,{work.add(it)},{delivery.add(it)})
        model.open("https://example.org");work.removeAt(0)();model.close();work.removeAt(0)();delivery.removeAt(0)()
        assertEquals(listOf("browser"),repository.stopped);assertEquals("",model.state.session)
    }
}
